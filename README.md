...
| Other platforms | ❌ Unsupported | Returns `ErrUnsupported` error  |

**Note:** On unsupported platforms (OpenBSD, NetBSD, Solaris, iOS, Android, etc.), the package compiles successfully but all clipboard operations return `ErrUnsupported`.
...
- **Other platforms**: Stub implementation that returns `ErrUnsupported` for all operations
